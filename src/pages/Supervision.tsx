              ...
              <div key={index} className="card">
                <div className="flex items-start">
                  ...
                  <div className="ml-4 flex-1">
                    ...
                    <p className="text-gray-700 mb-2"><strong>Grant:</strong> {postdoc.grant}</p>
                    <p className="text-gray-700 mb-2"><strong>Project:</strong> {postdoc.project}</p>
              ...
              <div key={index} className="card bg-white">
                <div className="flex items-start">
                  ...
                  <div className="ml-4 flex-1">
                    ...
                    <p className="text-gray-700 mb-2"><strong>Grant:</strong> {phd.grant}</p>
                    {phd.project && (
                      <p className="text-gray-700 mb-2"><strong>Project:</strong> {phd.project}</p>
                    )}
                    {phd.thesis && (
                      <p className="text-gray-700 mb-2"><strong>Thesis:</strong> {phd.thesis}</p>
                    )}
                    <div className="flex items-center text-sm text-gray-600">
                      {phd.period && (
                        <>
            ...
            {cvData.supervision.visitingPhds.map((vphd, index) => (
              <div key={index} className="card">
                <div className="flex items-start">
                  ...
                  <div className="ml-4 flex-1">
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">{vphd.name}</h3>
                    <p className="text-gray-700 mb-1"><strong>From:</strong> {vphd.from}</p>
                    <p className="text-primary mb-2">{vphd.institution}</p>
                    {vphd.project && (
                      <p className="text-gray-700 mb-2"><strong>Project:</strong> {vphd.project}</p>
                    )}
                    <div className="flex items-center text-sm text-gray-600">
                      <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {cvData.supervision.masters.map((ms, index) => (
              <div key={index} className="card bg-white">
                <h3 className="text-lg font-semibold text-gray-900 mb-2">{ms.name}</h3>
                <p className="text-primary text-sm mb-2">{ms.institution}</p>
                {ms.thesis && (
                  <p className="text-gray-700 text-sm mb-2"><strong>Thesis:</strong> {ms.thesis}</p>
                )}
                <p className="text-xs text-gray-600">{ms.period}</p>
              </div>
            ))}
            {cvData.supervision.interns.map((intern, index) => (
              <div key={index} className="card bg-white">
                <h3 className="text-lg font-semibold text-gray-900 mb-2">{intern.name}</h3>
                <p className="text-primary text-sm mb-2">{intern.institution}</p>
                <p className="text-gray-700 text-sm mb-2"><strong>Grant:</strong> {intern.grant}</p>
                {intern.project && (
                  <p className="text-gray-700 text-sm mb-2"><strong>Project:</strong> {intern.project}</p>
                )}
                <p className="text-xs text-gray-600">{intern.period}</p>
              </div>
            ))}
          </div>
        </div>
